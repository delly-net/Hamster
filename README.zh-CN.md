...
`isSystem` 标记系统自动创建的账户（当前即期初账本账户）。系统账户与普通账户同在列表中，
但**不可由用户创建**，也**不会因改名或改类型而失去系统身份**——身份记在标记列上，不靠名称或类型反查。

| 接口 | 鉴权 | 说明 |
|---|---|---|
| `GET /api/accounts?includeInactive=false` | Bearer | 当前账套内我可见的账户（管理员为全部），`balance` 为派生余额 |
| `POST /api/accounts` | Bearer | 新建（201）；请求体 `{ name, scope, type, initialBalance }`，个人账户的归属人强制为当前登录者；期初金额非 0 时同时落成一笔期初交易 |
| `PUT /api/accounts/{id}` | Bearer | 修改名称/类型（204）；归属范围、归属人、所属账套与**期初金额**一经创建均不可修改 |
