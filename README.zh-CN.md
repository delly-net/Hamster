...
| `PUT /api/accounts/{id}` | Bearer | 只修改名称（204），请求体仅 `{ name }`；归属范围、归属人、所属账套、**账户类型**与**期初金额**一经创建均不可修改。请求里带上 `type` 也不会被读取 |
...
交易头的 `type` 有三种取值，**三种都有真实的写入路径**（不留只占位的枚举值）：
...
期初余额只能由系统生成，手工传 `OpeningBalance` 一律 400。
...
三种记账都落在同一个账户上，故一个账套恒只有一个。它也**不是**「期初专用」，
名称里的「期初」二字只记录了它诞生于期初入账这一历史——它从不出现在任何界面上，改名与否不影响使用。

###### 记一笔收入与支出

收支记账只有一个写入端点，不提供单笔交易查询：

| 端点 | 认证 | 说明 |
|---|---|---|
| `POST /api/transactions` | Bearer | 记一笔收入或支出（201）。请求体 `{ type, accountId, amount, occurredAt, summary, remark }`：`type` 只能是 `Income` / `Expense`（字符串，传数字或 `OpeningBalance` 一律 400）；`amount` 须 `> 0` 且至多两位小数；`occurredAt` 为 ISO 8601 时间，取**业务发生时间**（可补记往日）；`summary` 必填（≤128）、`remark` 可选（≤256）。返回落库后的交易（含 `id` / `accountName`） |

该端点同时落成两条明细——所选账户与账套的账本账户各一条、金额相等方向相反，
**不需要也不接受**请求方指定对手方。方向由 `type` 推出（见上表），故 `amount` 恒为正。
