...
/* 33rem = 528px = #64 定下的 44rem 的 3/4（用户要求宽度减 1/4）。
   **代价是字段区由两列退回单列，这是算得出来的**：`.fields` 放下两列 20rem 需要
   2 × 320px + 0.7rem 列间距 + 两侧 1.35rem 内边距 ≈ 43.4rem 面板宽（#64 的 44rem 正是从这条算式来的），
   而 33rem 的面板内容宽只有 30.3rem，auto-fit 只能放一列。
   **这个取舍是刻意的**：宁可退回单列，也不把 `.fields` 的列宽下限由 20rem 压低去硬挤两列
   ——那会把账户选择框的候选行压窄，#60 定下的「余额 7 位下账户名可见 8 个汉字」随之退化。
   仍比账套选择弹窗（26rem）宽：**全站弹窗不再只有一种尺寸**，宽度跟着内容走。 */
.panel {
  width: 100%;
  max-width: 33rem;
  max-height: 80vh;
...
/* 栅格写法与记账表单 `EntryRecordForm` 同源：容器够宽才并两列、不够宽就退回单列，无需媒体查询。
   **本弹窗在 33rem 面板下恒为单列**（内容宽 30.3rem < 两列所需的 40.7rem，算式见 `.panel`）——
   保留这段栅格而不是改成纵向 flex 是刻意的：容器宽度若再变，行为自动跟着走，不必回来重写；
   但别照着注释去找第二列，这里现在只有一列。
   列宽下限 20rem（320px）的算式见 `EntryRecordForm`：账户选择框的候选行要装得下
