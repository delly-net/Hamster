...
export const useAccountsStore = defineStore('accounts', () => {
   ...
   * 清空列表。
   *
   * 两个调用方、同一件事——**手上的这份账户列表已不再是事实**：
   * 1. 退出登录或账套失效（避免残留上一账套的账户）；
   * 2. **记账提交成功后**（见 `EntryRecordForm.vue`）：一笔交易必然改变相关账户的余额，
   *    且对手方按名新建时后端还会多出一个本账套的往来账户——而候选行右侧显示的就是那个余额，
   *    留着即是拿记账前的数去回答「这笔钱从哪出」。清空后由下一次需要候选时补拉。
   */
  function clear(): void {
    accounts.value = []
  }
