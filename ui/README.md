...
| 候选中的余额 | 每一候选行右侧显示该账户的**当前余额**，格式 `1,234.56 CNY`（千分位 + 固定两位小数 + 币种代码），与「归属范围 · 归属人」同格靠右、同色、`tabular-nums` 等宽对齐。余额是**附加信息**，一律中性呈现（负号表达符号），不参与类型主题色 |
...
- **余额在浮层里不做语义着色**：它与「归属范围 · 归属人」同为中性 meta 信息，负余额靠 `-` 号表达即可。记账页的类型主题色（收/支/转账三色）只覆盖页面标题、提交按钮与成功提示条，**账户选择浮层不在其列**；引入红/绿只会让一个辅助信息抢走金额输入框的视线。同理，负债账户余额为负是正常财务状态，不加「欠款」文案与警示样式。
