...
| 表格 | 列：序号（页内行号 `index + 1`）、发生时间、账户、**收入**、**支出**、**净额**（三列均右对齐 + `tabular-nums`，正数绿、负数红）、对手方、摘要（附弱化的交易类型）、**分类**、备注；空态 `colspan="10"` 且**带上本次查询的条件概要** |
| 本页小计 | `<tfoot>` 一行：`colspan="3"` 的「本页小计」+ 收入合计 + 支出合计 + 净额合计 + `colspan="4"` 留白。只统计**当页**（跨页合计会让标题名不副实），空页时不渲染 |
