...
面向**所有登录用户**的记账账户管理页（[`src/views/AccountView.vue`](src/views/AccountView.vue) + [`src/stores/accounts.ts`](src/stores/accounts.ts)），路由只标 `requiresAuth`——**不带** `requiresAdmin`，菜单项同样不设 `adminOnly`。页面提供**弹窗新建**（【新增】→ 表单 →【保存】落表）、账户列表与行内编辑，并支持停用/启用。新建走弹窗、编辑走行内是**有意的分工**：新建要一次填四个字段且是低频主操作，弹窗能把注意力收拢；编辑只改一个名称，行内更省事。

| 环节 | 行为 |
|---|---|
| 未选择账套 | 只显示「请先切换账套」的提示，**不渲染弹窗与表格**——账户一律挂在账套下，展示一个不属于任何账套的空列表只会误导 |
| 跟随账套切换 | `watch` 账套 store 的 `currentId`，变化即重新拉取；未选择时清空列表。**仅靠 `onMounted` 会残留上一账套的账户**；账套一变也顺手关掉新建弹窗，否则弹窗里留着的是上一个账套的填写内容 |
| 新建（弹窗） | 点【新增】弹出表单（【保存】落表、【取消】/`Esc`/点遮罩关闭），字段为名称 + 归属范围（个人/公共）+ 类型（**三选一**）+ 期初金额。归属人由后端强制为当前登录者，故表单**没有**归属人字段。期初金额非 0 时后端会同时落成一笔期初交易。每次打开都复位表单；**提交中禁止关闭**（请求已发出，此刻关掉会让人以为没建成功而重试） |
| 行内编辑 | **只可改名称**；账户类型、归属范围、期初金额均**不可改**（类型是账户的分类身份，归属范围个人 → 公共等于把私有数据公开给全账套，期初金额已落成一笔期初交易），故编辑态都不呈现这三者的可编辑控件 |
...
- **类型下拉只有三项（没有账本账户）**：账本账户由系统在期初入账时自动创建，后端既不返回它（列表与单条查询都过滤）、也拒绝手工新建（`POST` 传 `Ledger` 一律 400），故本页既看不到它也无法造出它，**无需为此写任何本地过滤或禁用逻辑**。`ACCOUNT_TYPE_OPTIONS` 与 `ACCOUNT_TYPE_LABELS` 分工不同：前者是「用户可选什么」（三项），后者是「接口可能回传什么」（后端枚举的完整镜像，故仍含 `Ledger`），**少一项不是遗漏**。
- **账户类型不可修改**：类型是账户的分类身份，既有流水都按它归类，换类型等于给历史流水换一套解释，故编辑态只读呈现、不提供控件（**连禁用的下拉也不给**——禁用控件仍会暗示「以后能改」）。后端 `PUT` 的请求体同样只有 `name`，带上 `type` 也不会被读取。这条限制还顺带消灭了「先建资金账户再改成账本账户」的绕过路径，后端因此不再需要在该端点上做类型校验。
- **行内编辑与只读列的一致性**：本页对「一经创建不可修改」的列（类型、期初金额）一律**只读文本呈现**，不呈现任何可编辑或禁用的控件；编辑态仅名称列变成输入框。
- **余额是派生值**：页面展示的余额来自后端的 `balance` 字段（该账户全部交易明细的有符号汇总，期初已计入其中）。**不要在前端用期初金额渲染余额列**，那会在有流水之后与后端脱节。
- **期初金额只读**：它已落成一笔期初交易，改写它等于篡改既成事实，故 `UpdateAccountPayload` 不含该字段，编辑态也只读呈现。`UpdateAccountPayload` 如今**只剩 `name`**：归属范围、归属人、期初金额与类型全部不可修改，后端 `AccountUpdateRequest` 与之一一对应。
