...
| 重置 | 【重置】把账户回到第一个候选、金额与摘要/备注/分类清空、时间回到此刻 |
...
| 表格 | **仅宽屏呈现**。列：序号（页内行号 `index + 1`）、发生时间、账户、**收入**、**支出**、**净额**（三列均右对齐 + `tabular-nums`，正数绿、负数红）、对手方、摘要（附弱化的交易类型）、**分类**、备注；空态 `colspan="10"` 且**带上本次查询的条件概要** |
| 本页小计 | **仅宽屏呈现**。`<tfoot>` 一行：`colspan="3"` 的「本页小计」+ 收入合计 + 支出合计 + 净额合计 + `colspan="4"` 留白。只统计**当页**（跨页合计会让标题名不副实），空页时不渲染。窄屏下同一组数字改由卡片流末尾的汇总条呈现，取值同为 `pageTotals` |
| 窄屏呈现（<1024px） | 表格与 `<tfoot>` 隐藏，改为**卡片流**（`ul.cards`，一条明细一张卡）+ 末尾**汇总条**（`.totals`）：卡片头部左侧是收支标签 + 交易类型、右侧是金额（即为该行**净额**，正绿负红），其下依次是「账户 → 对手方」「摘要」「时间 · 分类 · 备注」；空态与宽屏**逐字相同**（同一份 `emptyText`），空页时汇总条不渲染 |
