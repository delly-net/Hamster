...
| 新建 | 名称 + 归属范围（个人/公共）+ 类型（**三选一**）+ 期初金额。归属人由后端强制为当前登录者，故表单**没有**归属人字段。期初金额非 0 时后端会同时落成一笔期初交易 |
| 行内编辑 | 可改名称、类型；归属范围**不可改**（个人 → 公共等于把私有数据公开给全账套），**期初金额也不可改**（已落成一笔期初交易），故编辑态都不呈现这两个字段 |
...
- **类型下拉只有三项（没有账本账户）**：账本账户由系统在期初入账时自动创建，后端既不返回它（列表与单条查询都过滤）、也拒绝手工指定或变更（`POST` / `PUT` 传 `Ledger` 一律 400），故本页既看不到它也无法造出它，**无需为此写任何本地过滤或禁用逻辑**。`ACCOUNT_TYPE_OPTIONS` 与 `ACCOUNT_TYPE_LABELS` 分工不同：前者是「用户可选什么」（三项），后者是「接口可能回传什么」（后端枚举的完整镜像，故仍含 `Ledger`），**少一项不是遗漏**。
- **余额是派生值**：页面展示的余额来自后端的 `balance` 字段（该账户全部交易明细的有符号汇总，期初已计入其中）。**不要在前端用期初金额渲染余额列**，那会在有流水之后与后端脱节。
- **期初金额只读**：它已落成一笔期初交易，改写它等于篡改既成事实，故 `UpdateAccountPayload` 不含该字段，编辑态也只读呈现。
