...
| 功能菜单 | 数据源为 [`src/config/menu.ts`](src/config/menu.ts)，以**路由名**指向路由；`adminOnly: true` 的项（接口调试、用户管理、账套管理）仅管理员渲染；「首页」「账户管理」「账目明细」与末尾的「用户设置」面向所有登录用户 |
...
| 表格 | 列：序号（页内行号 `index + 1`）、发生时间、账户、方向（借/贷徽标）、金额（右对齐 + `tabular-nums`）、对手方、摘要（附弱化的交易类型）、备注；空态 `colspan="8"` 且**带上本次查询的条件概要** |
...
- **一行是一条复式明细，不是一笔交易**：一笔交易由借贷两条明细构成，当两侧都落在所选账户上时就会呈现**两行**。这不是重复数据，而是复式记账在界面上的样子——把同笔交易合并成一行需要引入「净额」概念，而那与「查明细」的目的相反。
- **金额恒为正，方向单独成列**：`amount` 由后端保证为正，增减看「借/贷」标签。前端**不折算带符号金额**——「方向 → 符号」的唯一换算点是后端的 `TransactionService.SumSignedAmountsAsync`，在列表里再算一次就是第二个语义源（「借」在不同账户类型上含义不同，一旦折算散落各处就会对不上）。
- **不做任何可见性过滤**：后端只返回「挂在我可见账户上」的明细，账户多选的候选也由后端过滤（他人在个人账户根本不在列表里）。本页**不做任何本地过滤**——前端的可见性只是体验层，篡改本地状态只会拿到一批 400 / 403。
- **账本账户不出现在账户多选里**：它是系统内部账户（每账套恰一个，由期初入账自动创建），后端从不返回它，故本页无需为此写过滤或禁用逻辑。它在明细里只作为期初分录的对手方出现，显示为「期初」而非「账本账户」——用户在明细里看到它时想问的是「这笔钱从哪来」，答案就是「期初」。
- **对手方分档呈现，前端不给「回退到 Id」的分支**：`Account` 档直接用 `counterpartyName`；`Ledger` 显示「期初」；`Hidden` 与 `None` 显示「—」。后端对不可见对手方**连主键都不回传**，故前端也不存在任何「拿到 Id 再想办法」的路径。
